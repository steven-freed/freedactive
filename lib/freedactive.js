...
var Freedactive = (function() {
    ...
    var jsx$Utils = (function() {
        ...
        function parse(comp, context) {
            ...
            if (comp.emptyElTag) {
                ...
                context = context.replace(comp.regexMatch, new window[comp.name]().markup);
    ...
    function Switch() {
        ...
        this.markup = '<div id="app-switch" style="${}"></div>'.$({ 0: style });
    ...
    function Link() {
        ...
        // erase custom props so standard props can be rendered
        ...
        this.markup = ('<a href="${path}" ${attr} \
            onclick="${handler}" style="${style}">${innerHTML}</a>').$({
